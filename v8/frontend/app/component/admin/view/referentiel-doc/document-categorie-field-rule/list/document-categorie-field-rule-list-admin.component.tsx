...
import Edit from '../edit/document-categorie-field-rule-edit-admin.component';
import Create from '../create/document-categorie-field-rule-create-admin.component';
import View from '../view/document-categorie-field-rule-view-admin.component';
...
const List = () => {
    ...
    const [items, setItems] = useState<DocumentCategorieFieldRuleDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<DocumentCategorieFieldRuleDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<DocumentCategorieFieldRuleDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<DocumentCategorieFieldRuleDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new DocumentCategorieFieldRuleCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<DocumentCategorieFieldRuleDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new DocumentCategorieFieldRuleCriteria());
        fetchItems(new DocumentCategorieFieldRuleCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await DocumentCategorieFieldRuleAdminService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: DocumentCategorieFieldRuleDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: DocumentCategorieFieldRuleDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: DocumentCategorieFieldRuleDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: DocumentCategorieFieldRuleDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await DocumentCategorieFieldRuleAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'DocumentCategorieFieldRule Deleted');
        } catch (error) {
            MessageService.showError(toast, 'DocumentCategorieFieldRule Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await DocumentCategorieFieldRuleAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'DocumentCategorieFieldRules Deleted');
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: DocumentCategorieFieldRuleDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage document categorie field rules</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as DocumentCategorieFieldRuleDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    <Column selectionMode="multiple" headerStyle={{ width: '4rem' }}> </Column>
                    
                    <Column field="code" header={t("documentCategorieFieldRuleCode")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("documentCategorieFieldRuleLibelle")} sortable></Column>
                    
                    
                    <Column field="expresion" header={t("documentCategorieFieldRuleExpresion")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete document categorie field rule <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected document categorie field rules?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
