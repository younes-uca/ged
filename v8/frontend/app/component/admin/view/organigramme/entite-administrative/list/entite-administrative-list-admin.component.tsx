...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<EntiteAdministrativeDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new EntiteAdministrativeCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<EntiteAdministrativeDto[]>>();
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await EntiteAdministrativeAdminService.findPaginatedByCriteria(criteria);
        ...
        <div className="col-12">
            <div className="card">
                    ...
                    <div className="search-container">
                        <div className="grid">
                            ...
                            <span className="mr-3 align-search-items mt-4">
                                <Dropdown id="4" value={criteria.utilisateur} options={utilisateurs} onChange={(e) => setCriteria({ ...criteria, utilisateur: e.target.value })} optionLabel="nom" filter showClear placeholder="Utilisateur" filterPlaceholder="Utilisateur"/>
                            </span>
                            <span className="mr-3 align-search-items mt-4">
                                <Dropdown id="5" value={criteria.entiteAdministrativeType} options={entiteAdministrativeTypes} onChange={(e) => setCriteria({ ...criteria, entiteAdministrativeType: e.target.value })} optionLabel="libelle" filter showClear placeholder="EntiteAdministrativeType" filterPlaceholder="EntiteAdministrativeType"/>
                            </span>
                        </div>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete entite administrative <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected entite administratives?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
