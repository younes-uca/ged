...
const List = () => {
    ...
    const [items, setItems] = useState<EntiteAdministrativeTypeDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<EntiteAdministrativeTypeDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<EntiteAdministrativeTypeDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<EntiteAdministrativeTypeDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new EntiteAdministrativeTypeCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<EntiteAdministrativeTypeDto[]>>();
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await EntiteAdministrativeTypeAdminService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: EntiteAdministrativeTypeDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: EntiteAdministrativeTypeDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: EntiteAdministrativeTypeDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: EntiteAdministrativeTypeDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await EntiteAdministrativeTypeAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'EntiteAdministrativeType Deleted');
        } catch (error) {
            MessageService.showError(toast, 'EntiteAdministrativeType Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await EntiteAdministrativeTypeAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'EntiteAdministrativeTypes Deleted');
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: EntiteAdministrativeTypeDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage entite administrative types</h5>
        ...
        <div className="col-12">
            <div className="card">
                    ...
                    <div className="search-container">
                        <div className="grid">
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                ...
                                <label htmlFor="1">{t("entiteAdministrativeTypeCode")}</label>
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <InputText id="2" value={criteria.libelle} onChange={(e) => setCriteria({ ...criteria, libelle: e.target.value })} />
                                <label htmlFor="2">{t("entiteAdministrativeTypeLibelle")}</label>
                            </span>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as EntiteAdministrativeTypeDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("entiteAdministrativeTypeCode")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("entiteAdministrativeTypeLibelle")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete entite administrative type <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected entite administrative types?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
