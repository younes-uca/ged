...
import Edit from '../edit/role-utilisateur-edit-admin.component';
import Create from '../create/role-utilisateur-create-admin.component';
import View from '../view/role-utilisateur-view-admin.component';
...
const List = () => {
    ...
    const [items, setItems] = useState<RoleUtilisateurDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<RoleUtilisateurDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<RoleUtilisateurDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<RoleUtilisateurDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new RoleUtilisateurCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<RoleUtilisateurDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new RoleUtilisateurCriteria());
        fetchItems(new RoleUtilisateurCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await RoleUtilisateurAdminService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: RoleUtilisateurDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: RoleUtilisateurDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: RoleUtilisateurDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: RoleUtilisateurDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await RoleUtilisateurAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'RoleUtilisateur Deleted');
        } catch (error) {
            MessageService.showError(toast, 'RoleUtilisateur Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await RoleUtilisateurAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'RoleUtilisateurs Deleted');
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: RoleUtilisateurDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage role utilisateurs</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as RoleUtilisateurDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    <Column selectionMode="multiple" headerStyle={{ width: '4rem' }}> </Column>
                    
                    <Column field="code" header={t("roleUtilisateurCode")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("roleUtilisateurLibelle")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete role utilisateur <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected role utilisateurs?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
