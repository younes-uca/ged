...
const List = () => {
    ...
    const [items, setItems] = useState<DocumentFieldStateDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<DocumentFieldStateDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<DocumentFieldStateDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<DocumentFieldStateDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new DocumentFieldStateCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<DocumentFieldStateDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new DocumentFieldStateCriteria());
        fetchItems(new DocumentFieldStateCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await DocumentFieldStateAdminService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: DocumentFieldStateDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: DocumentFieldStateDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: DocumentFieldStateDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: DocumentFieldStateDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await DocumentFieldStateAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'DocumentFieldState Deleted');
        } catch (error) {
            MessageService.showError(toast, 'DocumentFieldState Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await DocumentFieldStateAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'DocumentFieldStates Deleted');
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: DocumentFieldStateDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage document field states</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as DocumentFieldStateDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("documentFieldStateCode")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("documentFieldStateLibelle")} sortable></Column>
                    
                    
                    <Column field="style" header={t("documentFieldStateStyle")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete document field state <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected document field states?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
