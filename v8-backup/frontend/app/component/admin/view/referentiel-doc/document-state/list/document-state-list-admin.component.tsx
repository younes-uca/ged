...
const List = () => {
    ...
    const [items, setItems] = useState<DocumentStateDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<DocumentStateDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<DocumentStateDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<DocumentStateDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new DocumentStateCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<DocumentStateDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new DocumentStateCriteria());
        fetchItems(new DocumentStateCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await DocumentStateAdminService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: DocumentStateDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: DocumentStateDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: DocumentStateDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: DocumentStateDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await DocumentStateAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'DocumentState Deleted');
        } catch (error) {
            MessageService.showError(toast, 'DocumentState Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await DocumentStateAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'DocumentStates Deleted');
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: DocumentStateDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage document states</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as DocumentStateDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("documentStateCode")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("documentStateLibelle")} sortable></Column>
                    
                    
                    <Column field="style" header={t("documentStateStyle")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete document state <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected document states?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
