...
import Edit from '../edit/document-categorie-field-edit-agent.component';
import Create from '../create/document-categorie-field-create-agent.component';
import View from '../view/document-categorie-field-view-agent.component';
...
const List = () => {
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await DocumentCategorieFieldAgentService.findPaginatedByCriteria(criteria);
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const deleteItem = async () => {
        try{
            await DocumentCategorieFieldAgentService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
    ...
    const deleteSelectedItems = async () => {
        await DocumentCategorieFieldAgentService.deleteList(selectedItems);
                    ...
                    <div className="search-container">
                        <div className="grid">
                            <span className="mr-3 align-search-items mt-4">
                                <Dropdown id="1" value={criteria.field} options={fields} onChange={(e) => setCriteria({ ...criteria, field: e.target.value })} optionLabel="libelle" filter showClear placeholder="Field" filterPlaceholder="Field"/>
                            </span>
                            <span className="mr-3 align-search-items mt-4">
                                <Dropdown id="2" value={criteria.documentCategorie} options={documentCategories} onChange={(e) => setCriteria({ ...criteria, documentCategorie: e.target.value })} optionLabel="libelle" filter showClear placeholder="DocumentCategorie" filterPlaceholder="DocumentCategorie"/>
                            </span>
                            <span className="mr-3 align-search-items mt-4">
                                <Dropdown id="3" value={criteria.documentCategorieFieldRule} options={documentCategorieFieldRules} onChange={(e) => setCriteria({ ...criteria, documentCategorieFieldRule: e.target.value })} optionLabel="libelle" filter showClear placeholder="DocumentCategorieFieldRule" filterPlaceholder="DocumentCategorieFieldRule"/>
                            </span>
                        </div>
