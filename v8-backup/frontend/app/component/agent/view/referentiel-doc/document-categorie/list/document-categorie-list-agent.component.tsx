...
import Edit from '../edit/document-categorie-edit-agent.component';
import Create from '../create/document-categorie-create-agent.component';
import View from '../view/document-categorie-view-agent.component';
...
const List = () => {
    ...
    const [items, setItems] = useState<DocumentCategorieDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<DocumentCategorieDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<DocumentCategorieDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<DocumentCategorieDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new DocumentCategorieCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<DocumentCategorieDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new DocumentCategorieCriteria());
        fetchItems(new DocumentCategorieCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await DocumentCategorieAgentService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: DocumentCategorieDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: DocumentCategorieDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: DocumentCategorieDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: DocumentCategorieDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await DocumentCategorieAgentService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'DocumentCategorie Deleted');
        } catch (error) {
            MessageService.showError(toast, 'DocumentCategorie Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await DocumentCategorieAgentService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'DocumentCategories Deleted');
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: DocumentCategorieDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage document categories</h5>
        ...
        <div className="col-12">
            <div className="card">
                    ...
                    <div className="search-container">
                        <div className="grid">
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                ...
                                <label htmlFor="1">{t("documentCategorieCode")}</label>
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <InputText id="2" value={criteria.libelle} onChange={(e) => setCriteria({ ...criteria, libelle: e.target.value })} />
                                <label htmlFor="2">{t("documentCategorieLibelle")}</label>
                            </span>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as DocumentCategorieDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("documentCategorieCode")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("documentCategorieLibelle")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete document categorie <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected document categories?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
