...
class AbstractService<T extends BaseDto, C extends BaseCriteria> {
    ...
    uploadAndOcr(event:any)  {
        ...
        axios.post('http://localhost:8036/api/open/minio/upload/file/ged', formData).then(response => {
            console.log('Upload successful:', response.data);
        }).catch(error => {
            console.error('Upload error:', error);
        });

        axios.post('http://localhost:8036/api/open/ocr/', formData).then(response => {
