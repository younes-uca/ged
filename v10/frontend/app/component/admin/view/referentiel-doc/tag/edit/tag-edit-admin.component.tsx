...
type TagEditAdminType = {
    visible: boolean,
    onClose: () => void,
    showToast: React.Ref<Toast>,
    selectedItem: TagDto
    update: (item: TagDto) => void,
    t: TFunction
}
const Edit: React.FC<TagEditAdminType> = ({visible, onClose, showToast, selectedItem, update, t}) => {
    ...
    const editItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await TagAdminService.update(item);
            update(response.data);
            MessageService.showSuccess(showToast, 'Tag Updated');
            onClose();
            setSubmitted(false);
    }
};
    ...
    return(
    <Dialog visible={visible} style={{width: '70vw'}} header={t("tagTabPan")} modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog}>
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header={t("tagTabPan")}>
                <div className="formgrid grid">
                    <div className="field col-6">
                        <label htmlFor="code">{t("tagCode")}</label>
                    ...
                    <div className="field col-6">
                        <label htmlFor="libelle">{t("tagLibelle")}</label>
