...
type DocumentStateEditAdminType = {
    visible: boolean,
    onClose: () => void,
    showToast: React.Ref<Toast>,
    selectedItem: DocumentStateDto
    update: (item: DocumentStateDto) => void,
    t: TFunction
}
const Edit: React.FC<DocumentStateEditAdminType> = ({visible, onClose, showToast, selectedItem, update, t}) => {

    const emptyItem = new DocumentStateDto();
    const [submitted, setSubmitted] = useState(false);
    const [activeIndex, setActiveIndex] = useState<number>(0);
    const [activeTab, setActiveTab] = useState(0);
    const [item, setItem] = useState<DocumentStateDto>(selectedItem);
    ...
    const editItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await DocumentStateAdminService.update(item);
            update(response.data);
            MessageService.showSuccess(showToast, 'DocumentState Updated');
            onClose();
            setSubmitted(false);
    }
};
