...
const List = () => {
    ...
    const [items, setItems] = useState<EntiteAdministrativeDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<EntiteAdministrativeDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<EntiteAdministrativeDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<EntiteAdministrativeDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new EntiteAdministrativeCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<EntiteAdministrativeDto[]>>(null);
    ...
    const fetchItems = (criteria: EntiteAdministrativeCriteria) => {
        EntiteAdministrativeAdminService.findPaginatedByCriteria(criteria).then(({data}) => {
    ...
    const showEditModal = (item: EntiteAdministrativeDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: EntiteAdministrativeDto) => {
        setSelectedItem(item);
        setShowViewDialog(true);
    };

    const add = (item: EntiteAdministrativeDto) => {
        setItems([...items, item]);
    };

    const update = (updatedItem: EntiteAdministrativeDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: EntiteAdministrativeDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await EntiteAdministrativeAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'EntiteAdministrative Deleted');
        } catch (error) {
            MessageService.showError(toast, 'EntiteAdministrative Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await EntiteAdministrativeAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems([]);
        MessageService.showSuccess(toast, 'EntiteAdministratives Deleted');
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: EntiteAdministrativeDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage entite administratives</h5>
        ...
        <div className="col-12">
            <div className="card">
                    ...
                    <div className="search-container">
                        <div className="grid">
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                ...
                                <label htmlFor="1">{t("entiteAdministrativeCode")}</label>
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <InputText id="2" value={criteria.description} onChange={(e) => setCriteria({ ...criteria, description: e.target.value })} />
                                <label htmlFor="2">{t("entiteAdministrativeDescription")}</label>
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <InputText id="3" value={criteria.libelle} onChange={(e) => setCriteria({ ...criteria, libelle: e.target.value })} />
                                <label htmlFor="3">{t("entiteAdministrativeLibelle")}</label>
                            </span>
                            <span className="mr-3 align-search-items mt-4">
                                <Dropdown id="4" value={criteria.utilisateur} options={utilisateurs} onChange={(e) => setCriteria({ ...criteria, utilisateur: e.target.value })} optionLabel="nom" filter showClear placeholder={t("entiteAdministrativeUtilisateurPlaceHolder")} filterPlaceholder={t("entiteAdministrativeUtilisateurPlaceHolderFilter")} />
                            </span>
                            <span className="mr-3 align-search-items mt-4">
                                <Dropdown id="5" value={criteria.entiteAdministrativeType} options={entiteAdministrativeTypes} onChange={(e) => setCriteria({ ...criteria, entiteAdministrativeType: e.target.value })} optionLabel="libelle" filter showClear placeholder={t("entiteAdministrativeEntiteAdministrativeTypePlaceHolder")} filterPlaceholder={t("entiteAdministrativeEntiteAdministrativeTypePlaceHolderFilter")} />
                            </span>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as EntiteAdministrativeDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("entiteAdministrativeCode")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("entiteAdministrativeLibelle")} sortable></Column>
                    
                    
                    <Column field="utilisateur.nom" header={t("entiteAdministrativeUtilisateur")} sortable ></Column>
                    
                    
                    <Column field="entiteAdministrativeType.libelle" header={t("entiteAdministrativeEntiteAdministrativeType")} sortable ></Column>
