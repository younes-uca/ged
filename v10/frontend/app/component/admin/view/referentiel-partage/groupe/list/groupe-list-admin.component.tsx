...
const List = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<GroupeDto>(emptyItem);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new GroupeCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>(null);
    const dt = useRef<DataTable<GroupeDto[]>>(null);
    ...
    const fetchItems = (criteria: GroupeCriteria) => {
        GroupeAdminService.findPaginatedByCriteria(criteria).then(({data}) => {
        ...
        <div className="col-12">
            <div className="card">
                    ...
                    <div className="search-container">
                        <div className="grid">
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                ...
                                <label htmlFor="1">{t("groupeCode")}</label>
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <InputText id="2" value={criteria.libelle} onChange={(e) => setCriteria({ ...criteria, libelle: e.target.value })} />
                                <label htmlFor="2">{t("groupeLibelle")}</label>
                            </span>
                            <span className="mr-3 align-search-items mt-4">
                                <Dropdown id="3" value={criteria.utilisateur} options={utilisateurs} onChange={(e) => setCriteria({ ...criteria, utilisateur: e.target.value })} optionLabel="nom" filter showClear placeholder={t("groupeUtilisateurPlaceHolder")} filterPlaceholder={t("groupeUtilisateurPlaceHolderFilter")} />
                            </span>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as GroupeDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("groupeCode")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("groupeLibelle")} sortable></Column>
                    
                    
                    <Column field="utilisateur.nom" header={t("groupeUtilisateur")} sortable ></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("deleteGroupeConfirmationMessage")} <b>{item.libelle}</b>?</span>)}
                    </div>
                </Dialog>
