...
const List = () => {
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await GroupeService.findPaginatedByCriteria(criteria);
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const deleteItem = async () => {
        try{
            await GroupeService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'Groupe Deleted', life: 3000 });
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await GroupeService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'Groupes Deleted', life: 3000 });
    };
