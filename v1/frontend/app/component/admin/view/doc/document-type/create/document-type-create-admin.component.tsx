...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const [items, setItems] = useState<DocumentTypeDto[]>([list]);
    ...
    const isFormValid = () => {
       ...
       let errorMessages = new Array<string>();
        ...
        if(item.libelle == null || item.libelle == undefined || item.libelle == '')
    ...
    const saveItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await DocumentTypeService.save(item);
            add(response.data);
            MessageService.showSuccess(showToast, 'DocumentType Created');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
        }
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="DocumentType" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog} >
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="DocumentType">
