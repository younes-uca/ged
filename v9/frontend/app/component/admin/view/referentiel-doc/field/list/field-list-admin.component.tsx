...
import Edit from '../edit/field-edit-admin.component';
import Create from '../create/field-create-admin.component';
import View from '../view/field-view-admin.component';
...
const List = () => {
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await FieldAdminService.findPaginatedByCriteria(criteria);
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const deleteItem = async () => {
        try{
            await FieldAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
    ...
    const deleteSelectedItems = async () => {
        await FieldAdminService.deleteList(selectedItems);
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("deleteFieldConfirmationMessage")} <b>{item.libelle}</b>?</span>)}
                    </div>
                </Dialog>
