...
import Edit from '../edit/tag-edit-admin.component';
import Create from '../create/tag-create-admin.component';
import View from '../view/tag-view-admin.component';

const List = () => {

    const { t } = useTranslation();

    const emptyItem = new TagDto();
    const [items, setItems] = useState<TagDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<TagDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<TagDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<TagDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new TagCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<TagDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new TagCriteria());
        fetchItems(new TagCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await TagAdminService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: TagDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: TagDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: TagDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: TagDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await TagAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'Tag Deleted');
        } catch (error) {
            MessageService.showError(toast, 'Tag Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await TagAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'Tags Deleted');
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: TagDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage tags</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as TagDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    <Column selectionMode="multiple" headerStyle={{ width: '4rem' }}> </Column>
                    
                    <Column field="code" header={t("tagCode")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("tagLibelle")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("deleteTagConfirmationMessage")} <b>{item.libelle}</b>?</span>)}
                    </div>
                </Dialog>
