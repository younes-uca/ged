...
type AccessShareCreateAdminType = {
    visible: boolean,
    onClose: () => void,
    add: (item: AccessShareDto) => void,
    showToast: React.Ref<Toast>,
    list: AccessShareDto[],
    t: TFunction,
    service: AccessShareAdminService
}
const Create: React.FC<AccessShareCreateAdminType> = ({visible, onClose, add, showToast, list, t, service}) => {

    const emptyItem = new AccessShareDto();
    const [items, setItems] = useState<AccessShareDto[]>(list);
    ...
    const saveItem = () => {
        ...
        if (isFormValid()) {
            service.save(item).then(({data}) => {
                add(data);
                MessageService.showSuccess(showToast, 'Access share Created');
                onClose();
                setSubmitted(false);
            });
        }
    ...
    return (
        <Dialog visible={visible} style={{width: '70vw'}} header={t("accessShareTabPan")} modal className="p-fluid"
