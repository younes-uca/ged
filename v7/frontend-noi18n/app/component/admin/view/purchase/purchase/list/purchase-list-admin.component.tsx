...
const List = () => {
    ...
    const handleCancelClick = () => {
        ...
        fetchItems(new PurchaseCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await PurchaseAdminService.findPaginatedByCriteria(criteria);
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const deleteItem = async () => {
        try{
            await PurchaseAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'Purchase Deleted');
        } catch (error) {
            MessageService.showError(toast, 'Purchase Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await PurchaseAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'Purchases Deleted');
    };
        ...
        <div className="col-12">
            <div className="card">
                    ...
                    <div className="search-container">
                        <div className="grid">
                            ...
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                ...
                                <label htmlFor="2-1">Purchase date Min</label>
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                ...
                                <label htmlFor="2-2">Purchase date Max</label>
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as PurchaseDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="purchaseDate | date :'dd/MM/yyyy HH:mm'" header="Purchase date" sortable></Column>
                    ...
                    <Column field="client.fullName" header="Client" sortable ></Column>
