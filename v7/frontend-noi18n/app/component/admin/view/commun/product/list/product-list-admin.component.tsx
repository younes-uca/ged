...
const List = () => {
    ...
    const handleCancelClick = () => {
        ...
        fetchItems(new ProductCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await ProductAdminService.findPaginatedByCriteria(criteria);
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const deleteItem = async () => {
        try{
            await ProductAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'Product Deleted');
        } catch (error) {
            MessageService.showError(toast, 'Product Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await ProductAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'Products Deleted');
    };
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Card>
                    <div className="search-container">
                        <div className="grid">
                            ...
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <InputText id="2" value={criteria.reference} onChange={(e) => setCriteria({ ...criteria, reference: e.target.value })} />
                                <label htmlFor="2">Reference</label>
                            </span>
                        </div>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete product <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
