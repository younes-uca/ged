...
const List = () => {
           ...
           const fetchData = async () => {
            try {
                const [clientCategorysResponse ] = await Promise.all<ClientCategoryResponse>([
                    ClientCategoryAdminService.getList(),
                ]);
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await ClientAdminService.findPaginatedByCriteria(criteria);
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const deleteItem = async () => {
        try{
            await ClientAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'Client Deleted');
        } catch (error) {
            MessageService.showError(toast, 'Client Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await ClientAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'Clients Deleted');
    };
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Card>
                    <div className="search-container">
                        <div className="grid">
                            ...
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <Dropdown id="3" value={criteria.clientCategory} options={clientCategorys} onChange={(e) => setCriteria({ ...criteria, clientCategory: e.target.value })} optionLabel="reference" filter showClear placeholder="ClientCategory" />
                            </span>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as ClientDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="email" header={t("clientEmail")} sortable></Column>
                    
                    
                    <Column field="clientCategory.reference" header="Client category" sortable ></Column>
