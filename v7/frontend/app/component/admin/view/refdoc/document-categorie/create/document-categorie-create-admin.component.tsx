...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const saveItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await DocumentCategorieAdminService.save(item);
            add(response.data);
            MessageService.showSuccess(showToast, 'DocumentCategorie Created');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="DocumentCategorie" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog} >
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="Document categorie">
