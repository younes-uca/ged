...
import Edit from '../edit/document-field-edit-admin.component';
import Create from '../create/document-field-create-admin.component';
import View from '../view/document-field-view-admin.component';

const List = () => {

    const emptyItem = new DocumentFieldDto();
    const [items, setItems] = useState<DocumentFieldDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<DocumentFieldDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<DocumentFieldDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<DocumentFieldDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new DocumentFieldCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<DocumentFieldDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new DocumentFieldCriteria());
        fetchItems(new DocumentFieldCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await DocumentFieldAdminService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: DocumentFieldDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: DocumentFieldDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: DocumentFieldDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: DocumentFieldDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await DocumentFieldAdminService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'DocumentField Deleted');
        } catch (error) {
            MessageService.showError(toast, 'DocumentField Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await DocumentFieldAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'DocumentFields Deleted');
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: DocumentFieldDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage document fields</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <Card>
                    <div className="search-container">
                        <div className="grid">
                            ...
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <Dropdown id="2" value={criteria.document} options={documents} onChange={(e) => setCriteria({ ...criteria, document: e.target.value })} optionLabel="reference" filter showClear placeholder="Document" />
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <InputText id="3" value={criteria.value} onChange={(e) => setCriteria({ ...criteria, value: e.target.value })} />
                                <label htmlFor="3">Value</label>
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <Dropdown id="4" value={criteria.documentFieldState} options={documentFieldStates} onChange={(e) => setCriteria({ ...criteria, documentFieldState: e.target.value })} optionLabel="libelle" filter showClear placeholder="DocumentFieldState" />
                            </span>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as DocumentFieldDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="documentFieldState.libelle" header="Document field state" sortable body={(rowData)=>statusBodyTemplate(rowData.documentFieldState?.libelle,rowData.documentFieldState?.style)}></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete document field <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected document fields?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
