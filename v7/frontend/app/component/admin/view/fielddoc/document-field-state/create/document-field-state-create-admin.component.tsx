...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const saveItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await DocumentFieldStateAdminService.save(item);
            add(response.data);
            MessageService.showSuccess(showToast, 'DocumentFieldState Created');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
        }
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="DocumentFieldState" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog} >
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="Document field state">
