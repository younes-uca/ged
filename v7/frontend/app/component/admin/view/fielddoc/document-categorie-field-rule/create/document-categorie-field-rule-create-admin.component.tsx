...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const saveItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await DocumentCategorieFieldRuleAdminService.save(item);
            add(response.data);
            MessageService.showSuccess(showToast, 'DocumentCategorieFieldRule Created');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
        }
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="DocumentCategorieFieldRule" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog} >
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="Document categorie field rule">
                <div className="formgrid grid">
                    ...
                    <div className="field col-6">
                    <label htmlFor="expresion">Expresion</label>
