...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const [items, setItems] = useState<RoleUtilisateurDto[]>([list]);
    ...
    const saveItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await RoleUtilisateurAdminService.save(item);
            add(response.data);
            MessageService.showSuccess(showToast, 'RoleUtilisateur Created');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
        }
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="RoleUtilisateur" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog} >
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="Role utilisateur">
