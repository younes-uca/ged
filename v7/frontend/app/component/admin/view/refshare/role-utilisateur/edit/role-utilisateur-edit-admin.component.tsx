...
const Edit = ({visible, onClose, showToast, selectedItem, update}) => {
    ...
    const editItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await RoleUtilisateurAdminService.update(item);
            update(response.data);
            MessageService.showSuccess(showToast, 'RoleUtilisateur Updated');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
    }
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="RoleUtilisateur" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog}>
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="RoleUtilisateur">
