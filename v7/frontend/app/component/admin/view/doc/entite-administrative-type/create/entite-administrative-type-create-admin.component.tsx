...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const [items, setItems] = useState<EntiteAdministrativeTypeDto[]>([list]);
    ...
    const saveItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await EntiteAdministrativeTypeAdminService.save(item);
            add(response.data);
            MessageService.showSuccess(showToast, 'EntiteAdministrativeType Created');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
        }
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="EntiteAdministrativeType" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog} >
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="Entite administrative type">
