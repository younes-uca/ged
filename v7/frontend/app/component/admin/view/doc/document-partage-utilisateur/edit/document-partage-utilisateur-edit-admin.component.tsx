...
const Edit = ({visible, onClose, showToast, selectedItem, update}) => {
    ...
    const editItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await DocumentPartageUtilisateurAdminService.update(item);
            update(response.data);
            MessageService.showSuccess(showToast, 'DocumentPartageUtilisateur Updated');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
    }
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="DocumentPartageUtilisateur" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog}>
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="DocumentPartageUtilisateur">
