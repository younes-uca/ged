...
const Edit = ({visible, onClose, showToast, selectedItem, update}) => {
    ...
    const editItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await DocumentPartageGroupeAdminService.update(item);
            update(response.data);
            MessageService.showSuccess(showToast, 'DocumentPartageGroupe Updated');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
    }
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="DocumentPartageGroupe" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog}>
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="DocumentPartageGroupe">
                <div className="formgrid grid">
                    <div className="field col-6">
                        <label htmlFor="document">Document</label>
                        <Dropdown  id="documentDropdown"  value={item ? item.document : ''} options={documents} onChange={(e) => onDropdownChange(e, 'document')}   placeholder="Sélectionnez un document" filter filterPlaceholder="Rechercher un document" optionLabel="reference" />
                    </div>
                    <div className="field col-6">
                        <label htmlFor="groupe">Groupe</label>
                        <Dropdown  id="groupeDropdown"  value={item ? item.groupe : ''} options={groupes} onChange={(e) => onDropdownChange(e, 'groupe')}   placeholder="Sélectionnez un groupe" filter filterPlaceholder="Rechercher un groupe" optionLabel="libelle" />
                    </div>
                    <div className="field col-6">
                        <label htmlFor="dateShare">Date share</label>
                        <Calendar id="dateShare" value={item ? item.dateShare : ''} onChange={(e) => onInputDateChange(e, 'dateShare')} dateFormat="dd/mm/yy" showTime />
                    </div>
                    <div className="field col-6">
                        <label htmlFor="accessShare">Access share</label>
                        <Dropdown  id="accessShareDropdown"  value={item ? item.accessShare : ''} options={accessShares} onChange={(e) => onDropdownChange(e, 'accessShare')}   placeholder="Sélectionnez un accessShare" filter filterPlaceholder="Rechercher un accessShare" optionLabel="libelle" />
                    </div>
                </div>
            </TabPanel>
        </TabView>
