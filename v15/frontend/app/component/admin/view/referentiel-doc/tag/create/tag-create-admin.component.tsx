...
type TagCreateAdminType = {
    visible: boolean,
    onClose: () => void,
    add: (item: TagDto) => void,
    showToast: React.Ref<Toast>,
    list: TagDto[],
    service: TagAdminService,
    t: TFunction
}
const Create: React.FC<TagCreateAdminType> = ({visible, onClose, add, showToast, list, service, t}) => {
    ...
    const emptyItem = new TagDto();
        const {
            ...
            saveItem,
            formateDate
        } = useCreateHook<TagDto, TagCriteria>({list, emptyItem, onClose, add, showToast,service, isFormValid})
