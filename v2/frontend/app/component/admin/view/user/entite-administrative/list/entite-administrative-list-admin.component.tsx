...
const List = () => {
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await EntiteAdministrativeService.findPaginatedByCriteria(criteria);
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const deleteItem = async () => {
        try{
            await EntiteAdministrativeService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'EntiteAdministrative Deleted', life: 3000 });
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await EntiteAdministrativeService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'EntiteAdministratives Deleted', life: 3000 });
    };
                ...
                <Card>
                    <div className="search-container">
                        <div className="grid">
                            ...
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <Dropdown id="3" value={criteria.utilisateur} options={utilisateurs} onChange={(e) => setCriteria({ ...criteria, utilisateur: e.target.value })} optionLabel="name" filter showClear placeholder="Utilisateur" />
                            </span>
                        </div>
