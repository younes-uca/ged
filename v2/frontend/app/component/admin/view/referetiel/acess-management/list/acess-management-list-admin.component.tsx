...
import Edit from '../edit/acess-management-edit-admin.component';
import Create from '../create/acess-management-create-admin.component';
import View from '../view/acess-management-view-admin.component';

const List = () => {

    const emptyItem = new AcessManagementDto();
    const [items, setItems] = useState<AcessManagementDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<AcessManagementDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<AcessManagementDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<AcessManagementDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new AcessManagementCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<AcessManagementDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new AcessManagementCriteria());
        fetchItems(new AcessManagementCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await AcessManagementService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: AcessManagementDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: AcessManagementDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: AcessManagementDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: AcessManagementDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await AcessManagementService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'AcessManagement Deleted', life: 3000 });
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await AcessManagementService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'AcessManagements Deleted', life: 3000 });
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: AcessManagementDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage acess managements</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as AcessManagementDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete acess management <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected acess managements?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
