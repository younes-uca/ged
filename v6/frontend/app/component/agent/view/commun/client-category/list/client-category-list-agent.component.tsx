...
import {ClientCategoryAgentService} from 'app/controller/service/agent/ClientCategoryAgentService.service';
...
import Edit from '../edit/client-category-edit-agent.component';
import Create from '../create/client-category-create-agent.component';
import View from '../view/client-category-view-agent.component';

const List = () => {

    const emptyItem = new ClientCategoryDto();
    const [items, setItems] = useState<ClientCategoryDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<ClientCategoryDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<ClientCategoryDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<ClientCategoryDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new ClientCategoryCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<ClientCategoryDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new ClientCategoryCriteria());
        fetchItems(new ClientCategoryCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await ClientCategoryAgentService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: ClientCategoryDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: ClientCategoryDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: ClientCategoryDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: ClientCategoryDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await ClientCategoryService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showSuccess(toast, 'ClientCategory Deleted');
        } catch (error) {
            MessageService.showError(toast, 'ClientCategory Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await ClientCategoryAgentService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'ClientCategorys Deleted');
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: ClientCategoryDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage client categorys</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                    ...
                    <div className="search-container">
                        <div className="grid">
                            ...
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                ...
                                <label htmlFor="2">Code</label>
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <InputNumber id="3-1" value={criteria.socialIndexMin} onChange={(e) => setCriteria({ ...criteria, socialIndexMin: e.value })} mode="decimal" />
                                <label htmlFor="3-1">SocialIndex Min</label>
                            </span>
                            <span className="p-float-label mr-3 align-search-items mt-4">
                                <InputNumber id="3-2" value={criteria.socialIndexMax} onChange={(e) => setCriteria({ ...criteria, socialIndexMax: e.value })} mode="decimal" />
                                <label htmlFor="3-2">SocialIndex Max</label>
                            </span>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as ClientCategoryDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="socialIndex" header="SocialIndex" sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete client category <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected client categorys?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
