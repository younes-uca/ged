...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const [items, setItems] = useState<AcessManagementDto[]>([list]);
    ...
    const saveItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await AcessManagementAdminService.save(item);
            add(response.data);
            MessageService.showSuccess(showToast, 'AcessManagement Created');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
        }
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="AcessManagement" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog} >
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="AcessManagement">
