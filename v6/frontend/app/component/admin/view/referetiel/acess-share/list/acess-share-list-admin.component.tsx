...
const List = () => {
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await AcessShareAdminService.findPaginatedByCriteria(criteria);
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const deleteItem = async () => {
        try{
            ...
            setItems(_items);
            MessageService.showSuccess(toast, 'AcessShare Deleted');
        } catch (error) {
            MessageService.showError(toast, 'AcessShare Deletion Prob');
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await AcessShareAdminService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showSuccess(toast, 'AcessShares Deleted');
    };
