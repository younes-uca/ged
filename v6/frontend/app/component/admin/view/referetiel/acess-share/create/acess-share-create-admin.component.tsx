...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const [items, setItems] = useState<AcessShareDto[]>([list]);
    ...
    const saveItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await AcessShareAdminService.save(item);
            add(response.data);
            MessageService.showSuccess(showToast, 'AcessShare Created');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
        }
...
return(
    <Dialog visible={visible} style={{width: '70vw'}} header="AcessShare" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog} >
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="AcessShare">
