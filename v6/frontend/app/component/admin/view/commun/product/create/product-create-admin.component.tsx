...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const saveItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await ProductAdminService.save(item);
            add(response.data);
            MessageService.showSuccess(showToast, 'Product Created');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
        }
        ...
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="Product">
                <div className="formgrid grid">
                    ...
                    <InputNumber id="purchaseCost" value={item.purchaseCost} onChange={(e) => onInputNumerChange(e, 'purchaseCost')} />
                    </div>
                </div>
            </TabPanel>
        </TabView>
