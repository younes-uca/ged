...
const Edit = ({visible, onClose, showToast, selectedItem, update}) => {
    ...
    const editItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await ProductAdminService.update(item);
            update(response.data);
            MessageService.showSuccess(showToast, 'Product Updated');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
    }
        ...
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header="Product">
                <div className="formgrid grid">
                    ...
                    <div className="field col-6">
                        <label htmlFor="purchaseCost">PurchaseCost</label>
                        <InputNumber id="purchaseCost" value={item ? item.purchaseCost : 0} onChange={(e) => onInputNumerChange(e, 'purchaseCost')}/>
                    </div>
                </div>
            </TabPanel>
        </TabView>
