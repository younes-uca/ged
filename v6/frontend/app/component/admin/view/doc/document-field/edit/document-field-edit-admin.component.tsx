...
const Edit = ({visible, onClose, showToast, selectedItem, update}) => {
    ...
    const editItem = async () => {
        setSubmitted(true);
        if (isFormValid()) {
            const response = await DocumentFieldAdminService.update(item);
            update(response.data);
            MessageService.showSuccess(showToast, 'DocumentField Updated');
            onClose();
            setItem(emptyItem);
            setSubmitted(false);
    }
            ...
            <TabPanel header="DocumentField">
                <div className="formgrid grid">
                    ...
                    <div className="field col-6">
                        ...
                        <InputText id="value" value={item ? item.value : ''} onChange={(e) => onInputTextChange(e, 'value')} required autoFocus className={classNames({'p-invalid': submitted && !item.value})} />
                        {submitted && !item.value && <small className="p-invalid">Value is required.</small>}
                    </div>
                    <div className="field col-6">
                        <label htmlFor="documentFieldState">DocumentFieldState</label>
                        <Dropdown  id="documentFieldStateDropdown"  value={item ? item.documentFieldState : ''} options={documentFieldStates} onChange={(e) => onDropdownChange(e, 'documentFieldState')}   placeholder="Sélectionnez un documentFieldState" filter filterPlaceholder="Rechercher un documentFieldState" optionLabel="libelle" />
                    </div>
                </div>
            </TabPanel>
        </TabView>
