...
import Edit from '../edit/document-categorie-model-edit-agent.component';
import Create from '../create/document-categorie-model-create-agent.component';
import View from '../view/document-categorie-model-view-agent.component';
...
const List = () => {
    ...
    const service = new DocumentCategorieModelAgentService();
            ...
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            ...
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="3">{t("documentCategorieModel.documentCategoriePlaceHolder")}</label>
                                <Dropdown id="3" value={criteria.documentCategorie} options={documentCategories} onChange={(e) => setCriteria({ ...criteria, documentCategorie: e.target.value })} optionLabel="libelle" filter showClear/>
                            </div>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as DocumentCategorieModelDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("documentCategorieModel.code")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("documentCategorieModel.libelle")} sortable></Column>
                    
                    
                    <Column field="documentCategorie.libelle" header={t("documentCategorieModel.documentCategorie")} sortable ></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("deleteDocumentCategorieModelConfirmationMessage")} <b>{item.libelle}</b>?</span>)}
                    </div>
                </Dialog>
