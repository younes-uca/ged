...
import Edit from '../edit/document-index-element-state-edit-admin.component';
import Create from '../create/document-index-element-state-create-admin.component';
import View from '../view/document-index-element-state-view-admin.component';
...
const List = () => {
    ...
    const emptyItem = new DocumentIndexElementStateDto();
    const emptyCriteria = new DocumentIndexElementStateCriteria();
    const service = new DocumentIndexElementStateAdminService();
    ...
    } = useListHook<DocumentIndexElementStateDto, DocumentIndexElementStateCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{t("documentIndexElementState.code")}</label>
                                <InputText id="1" value={criteria.code} onChange={(e) => setCriteria({ ...criteria, code: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="2">{t("documentIndexElementState.libelle")}</label>
                                <InputText id="2" value={criteria.libelle} onChange={(e) => setCriteria({ ...criteria, libelle: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="3">{t("documentIndexElementState.style")}</label>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as DocumentIndexElementStateDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                    <Column selectionMode="multiple" headerStyle={{ width: '4rem' }}> </Column>
                    
                    <Column field="code" header={t("documentIndexElementState.code")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("documentIndexElementState.libelle")} sortable></Column>
                    
                    
                    <Column field="style" header={t("documentIndexElementState.style")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("deleteDocumentIndexElementStateConfirmationMessage")} <b>{item.libelle}</b>?</span>)}
                    </div>
                </Dialog>
