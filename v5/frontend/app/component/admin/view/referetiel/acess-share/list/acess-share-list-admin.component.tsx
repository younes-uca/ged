...
import Create from '../create/acess-share-create-admin.component';
import View from '../view/acess-share-view-admin.component';

const List = () => {

    const emptyItem = new AcessShareDto();
    const [items, setItems] = useState<AcessShareDto[]>([]);
    const [deleteItemDialog, setDeleteItemDialog] = useState(false);
    const [deleteItemsDialog, setDeleteItemsDialog] = useState(false);
    const [item, setItem] = useState<AcessShareDto>(emptyItem);
    const [selectedItems, setSelectedItems] = useState<AcessShareDto[]>([]);
    const [globalFilter, setGlobalFilter] = useState('');
    const [showCreateDialog, setShowCreateDialog] = useState<boolean>(false);
    const [showEditDialog, setShowEditDialog] = useState<boolean>(false);
    const [showViewDialog, setShowViewDialog] = useState<boolean>(false);
    const [selectedItem, setSelectedItem] = useState<AcessShareDto | null>(null);
    const [rows, setRows] = useState<number>(10);
    const [totalRecords, setTotalRecords] = useState(0);
    const [criteria, setCriteria] = useState(new AcessShareCriteria());
    const [first, setFirst] = useState(0);
    const toast = useRef<Toast>();
    const dt = useRef<DataTable<AcessShareDto[]>>();
    ...
    const handleCancelClick = () => {
        setCriteria(new AcessShareCriteria());
        fetchItems(new AcessShareCriteria());
    ...
    const fetchItems = async (criteria) => {
        try {
            const response = await AcessShareService.findPaginatedByCriteria(criteria);
    ...
    const showEditModal = (item: AcessShareDto) => {
        setSelectedItem(item);
        setShowEditDialog(true);
    };

    const showViewModal = (item: AcessShareDto) => {
        setSelectedItem(item);
    ...
    const update = (updatedItem: AcessShareDto) => {
   ...
   const hideDeleteItemsDialog = () => {
    ...
    const confirmDeleteItem = (item: AcessShareDto) => {
        setSelectedItem(item);
    ...
    const deleteItem = async () => {
        try{
            await AcessShareService.delete(selectedItem?.id);
            setDeleteItemDialog(false);
            setItem(emptyItem);
            let _items = items.filter((val) => val.id !== selectedItem?.id);
            setItems(_items);
            MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'AcessShare Deleted', life: 3000 });
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const deleteSelectedItems = async () => {
        await AcessShareService.deleteList(selectedItems);
        let _items = items.filter((val) => !selectedItems.includes(val));
        setItems(_items);
        setDeleteItemsDialog(false);
        setSelectedItems(null);
        MessageService.showToast(toast, { severity: 'success', summary: 'Successful', detail: 'AcessShares Deleted', life: 3000 });
    };
   ...
   const rightToolbarTemplate = () => {
    ...
    const actionBodyTemplate = (rowData: AcessShareDto) => {
       return ( <>
    ...
    const header = (
        ...
        <h5 className="m-0">Manage acess shares</h5>
    ...
    <div className="grid crud-demo">
        <div className="col-12">
            <div className="card">
                ...
                <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value as AcessShareDto[])} dataKey="id" className="datatable-responsive" globalFilter={globalFilter} header={header} responsiveLayout="scroll" >
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>Are you sure you want to delete acess share <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
                <Dialog visible={deleteItemsDialog} style={{width: '450px'}} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                    <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{fontSize: '2rem'}} />
                    {item && <span>Are you sure you want to delete the selected acess shares?</span>}
                    </div>
                </Dialog>
            </div>
        </div>
